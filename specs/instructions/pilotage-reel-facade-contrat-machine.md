...
| **Toucher `release.yml`, le pin `tauri-action`, le bloc `latest`** | B′-a est gaté PASS ; y toucher rouvrirait un lot fermé. *(Seule exception possible : si AR-P2(b) est retenu, l'étape d'obtention de la ressource doit exister **dans le build**, donc dans le workflow — c'est alors un ajout **déclaré**, gardé par le cliquet du bloc `latest` qui ne doit pas bouger.)* **[EXCEPTION EXERCÉE, 2026-09-05, correction post-gate FAIL — sans toucher `release.yml`]** : gate 🏹 Legolas (`docs/qualite/gate-pilotage-reel-contrat-machine.md`) a mesuré que le premier run CI échouait faute de cette étape (`resource path resources/cli doesn't exist`, quatre plateformes). Correctif : `beforeBuildCommand` de `tauri.conf.json` devient `"npm run embarquer && npm run build"` — Tauri exécute toujours `beforeBuildCommand` avant de packager, y compris quand c'est `tauri-action` qui invoque le build ; une seule commande gouverne donc local **et** CI, sans toucher `release.yml`. **Preuve** : `git diff` sur `release.yml`, `fixtures/tauri-action-pin.json`, `fixtures/bloc-latest.sha256` reste **vide** (les trois gardes `pin-tauri-action`/`release-matrice`/`bloc-latest` n'ont donc rien à rougir) ; reproduction sur ce poste — ressource déplacée + ancien `beforeBuildCommand` ⇒ échec identique au gate ; ressource toujours absente + nouveau `beforeBuildCommand` ⇒ `npm run tauri build -- --target aarch64-apple-darwin` télécharge, vérifie le `sha256` puis produit `.app` + `.dmg` (version embarquée `0.40.0` vérifiée dans le bundle). Garde neuve : `scripts/__tests__/ressource-avant-build.test.mjs` (contrefactuel rejoué). **Non couvert par cette correction, DÉCLARÉ gate humain / premier run CI** : le comportement de `tar` sur `windows-latest` (bsdtar présent depuis Windows 10 build 17063, mais non exécutable sur ce poste macOS) et les builds Linux/Windows/macOS Intel de la matrice. | — |
